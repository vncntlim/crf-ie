<html>
	...
	<body>
		...
		<!-- container -->
		<div class="jumbotron">
		  <h1 class="display-4">Pengujian!</h1>
		  <p class="lead">Anda dapat mengunggah file CSV untuk data uji pada formulir dibawah ini!</p>
		  <hr class="my-4">
		  <!-- form upload -->
		  <form method="post" action="" enctype="multipart/form-data">
			<div class="form-group">
			  <label for="exampleFormControlFile1">Unggah data uji.</label>
			  <input type="file" class="form-control-file" name="fileToUpload" id="fileToUpload" accept=".csv" required="required">
			</div>
			<button type="submit" class="btn btn-primary" name="proses">Uji!</button>
		  </form>
		</div>
	</body>
</html>
<?php
	if (isset($_POST['proses'])) {
		if ($_FILES["fileToUpload"]["size"] > 100000000) {
			...
			$namafiletujuan = "data_testing.csv";
			$url = 'http://localhost/crf-ei/';

			if(move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $namafiletujuan)){
				echo "<script>alert('Proses unggah data uji berhasil!');window.location.href='test.php';</script>";
			} else {
				echo "<script>alert('Proses unggah data uji gagal, silakan coba lagi!');</script>";
			}
		}
